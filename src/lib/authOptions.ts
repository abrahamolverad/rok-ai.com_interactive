...
function getGoogleProvider() {
  ...
  return id && secret ? [GoogleProvider({ clientId: id, clientSecret: secret })] : [];
}
...
export const authOptions: NextAuthOptions = {
  ...
  callbacks: {
    ...
    async session({ session, token }) { if (token.id && session.user) (session.user as { id: string }).id = token.id as string; return session; }
  },
