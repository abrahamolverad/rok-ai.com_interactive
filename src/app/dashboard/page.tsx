...
export default function DashboardPage() {
    ...
    // --- API URLs ---
    const dashboardApiUrl = (startDate && endDate) ? `/api/alpaca/dashboard?startDate=${startDate}&endDate=${endDate}` : null;
    ...
    const openPosCandlestickChartData = useMemo(() => {
        ...
        return [{
            ...
            increasing: { line: { color: '#FFFFFF' } }, // White
            decreasing: { line: { color: '#a855f7' } }  // rokPurple
        }];
    ...
    const handleExportCsv = () => {
        ...
        const csvData = filteredRealizedTrades.map(trade => ({ Symbol: trade.Symbol, Type: trade.Type, Quantity: trade.Qty, EntryPrice: trade.EntryPrice.toFixed(2), ExitPrice: trade.ExitPrice.toFixed(2), EntryTime: dayjs(trade.EntryTime).format('YYYY-MM-DD HH:mm:ss'), ExitTime: dayjs(trade.ExitTime).format('YYYY-MM-DD HH:mm:ss'), PnL: trade.Pnl.toFixed(2), ExitOrderID: trade.ExitOrderID }));
    ...
    // --- Helper Functions ---
    const formatPnl = (pnl: number | null | undefined) => { const fc = formatCurrency(pnl); if (pnl === null || typeof pnl === 'undefined' || isNaN(pnl)) return <span className="text-gray-500">{fc}</span>; return (<span className={pnl >= 0 ? 'text-green-500' : 'text-red-500'}>{fc}</span>); };
        ...
        <div className="p-4 md:p-6 space-y-6 bg-black text-rokIvory min-h-screen">
             ...
             <div className="flex justify-between items-center mb-6">
                <div className="flex items-center space-x-4">
                    <img src="/Logo_RokAi.jpg" alt="RokAi Logo" className="h-16 w-auto rounded" />
                    <h1 className="text-3xl font-bold text-rokIvory">Trading Dashboard</h1>
                </div>
            </div>

            {/* --- Filters --- */}
             <div className="mb-6 p-4 bg-rokGrayDark shadow-lg rounded border border-rokGrayBorder">
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                     <div>
                        <label htmlFor="startDate" className="block text-sm font-medium text-rokGraySubtle mb-1">Date Range:</label>
                        <div className="flex flex-wrap items-center gap-2">
                            <input type="date" id="startDate" value={startDate} onChange={(e) => setStartDate(e.target.value)} className="border border-rokGrayBorder bg-rokGrayInput text-rokGrayText p-2 rounded text-sm focus:ring-rokPurple focus:border-rokPurple" max={endDate || dayjs().format('YYYY-MM-DD')} />
                            <span className="text-gray-500">to</span>
                            <input type="date" id="endDate" value={endDate} onChange={(e) => setEndDate(e.target.value)} className="border border-rokGrayBorder bg-rokGrayInput text-rokGrayText p-2 rounded text-sm focus:ring-rokPurple focus:border-rokPurple" min={startDate || undefined} max={dayjs().format('YYYY-MM-DD')} />
                            <button onClick={() => mutateDashboard()} disabled={dashboardIsLoading} className="px-3 py-2 bg-rokPurple text-white rounded hover:bg-rokPurple/80 text-sm disabled:opacity-50 disabled:cursor-not-allowed" title="Load data for selected range"> {dashboardIsLoading ? 'Loading...' : 'Load Data'} </button>
                        </div>
                     </div>
                     <div>
                        <label htmlFor="symbolFilter" className="block text-sm font-medium text-rokGraySubtle mb-1">Filter by Symbol:</label>
                        <select id="symbolFilter" value={selectedSymbol} onChange={handleSymbolFilterChange} className="border border-rokGrayBorder bg-rokGrayInput text-rokGrayText p-2 rounded text-sm focus:ring-rokPurple focus:border-rokPurple w-full md:w-auto" >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
                <div className="p-4 bg-rokGrayDark shadow-lg rounded border border-rokGrayBorder text-center">
                    <div className="text-sm font-medium text-rokGraySubtle uppercase tracking-wider">Total Realized P&L</div>
                    <div className={`text-2xl font-semibold mt-1 ${dashboardData.totalRealizedPL >= 0 ? 'text-green-500' : 'text-red-500'}`}>
                 ...
                 <div className="p-4 bg-rokGrayDark shadow-lg rounded border border-rokGrayBorder text-center">
                    <div className="text-sm font-medium text-rokGraySubtle uppercase tracking-wider">Unrealized P&L</div>
                     <div className={`text-2xl font-semibold mt-1 ${dashboardData.unrealizedPL >= 0 ? 'text-green-500' : 'text-red-500'}`}>
             ...
             {/* --- Daily P&L Bar Chart --- */}
            <div className="p-4 bg-rokGrayDark shadow-lg rounded border border-rokGrayBorder mb-6">
                 ...
                 {dailyPnlChartData.length > 0 && dailyPnlChartData[0].x.length > 0 ? (
                     <Plot data={dailyPnlChartData as any} layout={{ autosize: true, paper_bgcolor: 'rgba(0,0,0,0)', plot_bgcolor: 'rgba(0,0,0,0)', font: { color: '#f8f8f5' }, xaxis: { title: { text: 'Date', font: { color: '#9ca3af'} }, type: 'date', tickformat: '%Y-%m-%d', automargin: true, gridcolor: '#374151', linecolor: '#4b5563', zerolinecolor: '#4b5563', tickfont: { color: '#9ca3af'} }, yaxis: { title: { text: 'Daily P&L ($)', font: { color: '#9ca3af'} }, automargin: true, tickprefix: '$', gridcolor: '#374151', linecolor: '#4b5563', zerolinecolor: '#4b5563', tickfont: { color: '#9ca3af'} }, bargap: 0.2, uniformtext: { minsize: 9, mode: 'show' }, margin: { l: 60, r: 30, t: 50, b: 50 }, hovermode: 'x unified' }} useResizeHandler={true} className="w-full h-[350px] md:h-[450px]" config={{ responsive: true, displayModeBar: false }} />
            ...
            <div className={`p-4 bg-rokGrayDark shadow-lg rounded border border-rokGrayBorder mb-6 ${!openPositionChartSymbol ? 'hidden' : ''}`}>
                 <div className="flex justify-between items-center mb-3">
                    <h2 className="text-xl font-semibold text-rokIvory">Intraday Chart (15min, ~24h): {openPositionChartSymbol}</h2>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
                ...
                <div className="p-4 bg-rokGrayDark shadow-lg rounded border border-rokGrayBorder">
                    <h2 className="text-xl font-semibold text-rokIvory mb-3">Open Positions ({dashboardData.openPositions?.length ?? 0})</h2>
                    <div className="overflow-x-auto"> {dashboardData.openPositions?.length > 0 ? ( <table className="min-w-full divide-y divide-rokGrayBorder text-sm"> <thead className="bg-gray-700"> <tr> <th className="px-4 py-2 text-left font-medium text-rokGrayText uppercase tracking-wider">Symbol</th> <th className="px-4 py-2 text-right font-medium text-rokGrayText uppercase tracking-wider">Qty</th> <th className="px-4 py-2 text-right font-medium text-rokGrayText uppercase tracking-wider">Avg Entry</th> <th className="px-4 py-2 text-right font-medium text-rokGrayText uppercase tracking-wider">Current</th> <th className="px-4 py-2 text-right font-medium text-rokGrayText uppercase tracking-wider">Market Value</th> <th className="px-4 py-2 text-right font-medium text-rokGrayText uppercase tracking-wider">Unrealized P&L</th> </tr> </thead> <tbody className="bg-rokGrayDark divide-y divide-rokGrayBorder"> {dashboardData.openPositions.map((pos, index) => ( <tr key={`${pos.Symbol}-${index}`} className="hover:bg-gray-800">
                                            ...
                                            <td className="px-4 py-2 whitespace-nowrap text-right text-rokGrayText">{pos.Qty}</td> <td className="px-4 py-2 whitespace-nowrap text-right text-rokGrayText">{formatCurrency(pos['Avg Entry Price'])}</td> <td className="px-4 py-2 whitespace-nowrap text-right text-rokGrayText">{formatCurrency(pos['Current Price'])}</td> <td className="px-4 py-2 whitespace-nowrap text-right text-rokGrayText">{formatCurrency(pos['Market Value'])}</td> <td className="px-4 py-2 whitespace-nowrap text-right">{formatPnl(pos['Unrealized P&L'])}</td> </tr> ))} </tbody> </table> ) : ( <p className="text-gray-500 text-center py-5">No open positions.</p> )} </div>
                </div>
                ...
                <div className="p-4 bg-rokGrayDark shadow-lg rounded border border-rokGrayBorder">
                    ...
                    <div className="overflow-x-auto max-h-[400px]"> {filteredRealizedTrades.length > 0 ? ( <table className="min-w-full divide-y divide-rokGrayBorder text-sm">
                        <thead className="bg-gray-700 sticky top-0 z-10">
                            <tr>
                                ...
                                <th className="px-4 py-2 text-left font-medium text-rokGrayText uppercase tracking-wider">Order ID</th>
                            </tr>
                        </thead>
                        <tbody className="bg-rokGrayDark divide-y divide-rokGrayBorder">
                            {filteredRealizedTrades .sort((a, b) => new Date(b.ExitTime).getTime() - new Date(a.ExitTime).getTime()) .map((trade, index) => (
                                <tr key={`${trade.Symbol}-${trade.ExitTime}-${index}`} className="hover:bg-gray-800">
                                    ...
                                    <td className="px-4 py-2 whitespace-nowrap text-rokGrayText text-xs truncate" title={trade.ExitOrderID}>{trade.ExitOrderID}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table> ) : ( <p className="text-gray-500 text-center py-10">No closed trades found for {selectedSymbol === 'ALL' ? 'all symbols' : selectedSymbol} in the selected period.</p> )} </div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                 <div className="p-4 bg-rokGrayDark shadow-lg rounded border border-rokGrayBorder">
                     <h2 className="text-xl font-semibold text-green-500 mb-3">Top 10 Winning Symbols (Overall)</h2>
                     {aggregatedTopWinners.length > 0 ? ( <ul className="space-y-2 text-sm"> {aggregatedTopWinners.map((agg, i) => ( <li key={`win-agg-${agg.Symbol}-${i}`} className="flex justify-between items-center border-b border-rokGrayBorder pb-1"> <span className="text-rokGrayText">
                                        <span className="font-medium text-rokIvory">{agg.Symbol}</span>
                                        <span className="text-xs text-gray-500 ml-1">({agg.TradeCount} trade{agg.TradeCount !== 1 ? 's' : ''})</span>
                                     </span> <span className="font-medium text-green-500">{formatCurrency(agg.TotalPnl)}</span> </li> ))} </ul> ) : ( <p className="text-gray-500 text-center py-5">No winning trades found in the selected period.</p> )}
                </div>
                 <div className="p-4 bg-rokGrayDark shadow-lg rounded border border-rokGrayBorder">
                     <h2 className="text-xl font-semibold text-red-500 mb-3">Top 10 Losing Symbols (Overall)</h2>
                    {aggregatedTopLosers.length > 0 ? ( <ul className="space-y-2 text-sm"> {aggregatedTopLosers.map((agg, i) => ( <li key={`loss-agg-${agg.Symbol}-${i}`} className="flex justify-between items-center border-b border-rokGrayBorder pb-1"> <span className="text-rokGrayText">
                                        <span className="font-medium text-rokIvory">{agg.Symbol}</span>
                                        <span className="text-xs text-gray-500 ml-1">({agg.TradeCount} trade{agg.TradeCount !== 1 ? 's' : ''})</span>
                                     </span> <span className="font-medium text-red-500">{formatCurrency(agg.TotalPnl)}</span> </li> ))} </ul> ) : ( <p className="text-gray-500 text-center py-5">No losing trades found in the selected period.</p> )}
                </div>
            </div>
