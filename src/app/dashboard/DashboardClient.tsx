...
interface EquityPoint {
  ...
  pnl: number; // cumulative P&L at that date
}
  ...
  const renderSummaryCard = (
    ...
    <Card className="flex-1 text-center shadow-md">
      <CardContent className="py-6">
        <p className="text-sm uppercase tracking-wide mb-2 text-gray-400">{title}</p>
        <p className="text-2xl font-semibold">{value.toLocaleString("en-US", { style: "currency", currency: "USD" })}</p>
  ...
  return (
    <div className="min-h-screen bg-black text-white p-4 md:p-8 space-y-8">
      ...
      <header className="flex items-center justify-between flex-wrap gap-4">
        ...
        {/* Replace with your real SVG/logo once available */}
      ...
      <section className="flex flex-wrap gap-4 items-end">
        ...
        <Button
          ...
          onClick={() => window.location.assign(`/api/exportCsv?from=${from}&to=${to}&strategy=${strategy}&symbol=${symbol}`)}
        >
          Export CSV
        </Button>
      </section>
      ...
      {equityCurve.length > 0 && (
        <section className="w-full h-72 bg-white/5 p-4 rounded-2xl shadow-inner">
          ...
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={equityCurve} margin={{ top: 10, right: 20, left: 0, bottom: 0 }}>
              ...
              <tbody>
                {positions.map((pos) => (
                  <tr key={pos.symbol} className="border-b border-white/10 hover:bg-white/10">
                    ...
                    <td className={`p-2 text-right ${pos.unrealized >= 0 ? "text-green-400" : "text-red-400"}`}>
                      {pos.unrealized.toLocaleString("en-US", { style: "currency", currency: "USD" })}
                    </td>
                  </tr>
                ))}
              </tbody>
