...
export default function DashboardClient() {
  ...
  const [strategy, setStrategy] = useState<string>("day");
  const [start, setStart] = useState<string>(dayjs().subtract(30, "day").format("YYYY-MM-DD"));
  ...
  const renderPositions = () => {
    if (!dashboard?.openPositions?.length) return <p>No open positions</p>;
    return (
  ...
  const renderClosedTrades = () => {
    if (!dashboard?.closedTrades?.length) return <p>No closed trades</p>;
    return (
      <table className="mt-2 w-full text-sm">
        <thead>
          <tr className="border-b border-neutral-600">
  ...
  return (
    <div className="mx-auto max-w-5xl p-4 text-neutral-50">
      ...
      <div className="flex flex-wrap gap-4 items-end mb-8">
        ...
        <div className="flex flex-col">
          <label className="text-sm mb-1">Symbol</label>
          <input
            type="text"
        ...
        <div className="flex flex-col">
          <label className="text-sm mb-1">Strategy</label>
          <select
            value={strategy}
